...
def create_image_lists(testing_percentage, validation_percentage):
    ...
    for sub_dir in sub_dirs:  # 遍历目录数组，每次处理一种
        print ('文件夹为：'+sub_dir)#输出读取文件夹的路径，以填入strings
        if is_root_dir:
            is_root_dir = False
            continue
...
#image_lists:给出所有图片信息
#image_dir:图片根目录
#lable_name:给定类别名称
#index：获取图片编号
#category：指定图片来自训练、测试、验证数据集中的哪一个
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor, bottleneck_tensor):
    ...
    :return:
    """
    #【实际就是将当前图片作为输入计算瓶颈张量的值，即该图片新的特征向量】
    bottleneck_values = sess.run(bottleneck_tensor, {image_data_tensor: image_data})
    #处理结果是一个四维数组，将其压缩成一维数组
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    if not os.path.exists(bottleneck_path):  # 如果特征向量文件不存在，就计算出特征向量，并保存到文件
        # 获取图片原始路径
        image_path = get_image_path(image_lists, INPUT_DATA, label_name, index, category)
        # 获取图片内容
        image_data = gfile.FastGFile(image_path, 'rb').read()#虽然输入图片大小不一致，得到的img_data大小也不一致，但都能生成一个2048的特征向量
        # 计算图片特征向量
...
# 获取全部的测试数据，最终测试的时候需要在所有的测试数据上计算准确率
...
def main():
    ...
    with tf.Session(graph=graph) as sess:
        ...
        with tf.gfile.FastGFile("ckpt/nn.pb", mode='wb') as f:
